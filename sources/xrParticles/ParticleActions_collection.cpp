//==================================== BlackHoleEngine ============================================
// Projekt		: Particles
// Module		: ParticleActions_collection.cpp
// Author		: Anahoret
// Description	: 
//================================== SovaHiBlack© - 2022 ==========================================
...
void PAPI::PAAvoid::Execute(SParticleEffect* effect, f32 dt)
{
	...
	switch (position.type)
	{
		case ePDPlane:
		{
			if (look_ahead < P_MAXFLOAT)
			{
				...
				{
					SParticle& m = effect->particles[i];
					...
					{
						f32 vm = m.vel.length();
						CParticleVector Vn = m.vel / vm;
						// f32 dot = Vn * position.p2;

						CParticleVector tmp = (position.p2 * (magdt / (dist * dist + epsilon))) + Vn;
		...
		case ePDRectangle:
		{
			...
			CParticleVector& u = position.u;
			CParticleVector& v = position.v;

			// The normalized bases are needed inside the loop.
			CParticleVector un = u / position.radius1Sqr;
			CParticleVector vn = v / position.radius2Sqr;
			...
			CParticleVector s1((v.y * wz - v.z * wy), (v.z * wx - v.x * wz), (v.x * wy - v.y * wx));
			s1 *= det;
			CParticleVector s2((u.y * wz - u.z * wy), (u.z * wx - u.x * wz), (u.x * wy - u.y * wx));
			s2 *= -det;
			...
			for (u32 i = 0; i < effect->p_count; i++)
			{
				...
				CParticleVector pnext(m.pos + m.vel * dt * look_ahead);
				...
				CParticleVector phit(m.pos + m.vel * t);

				// Offset from origin in plane, p - origin
				CParticleVector offset(phit - position.p1);
				...
				CParticleVector uofs = (un * (un * offset)) - offset;
				f32 udistSqr = uofs.length2();
				CParticleVector vofs = (vn * (vn * offset)) - offset;
				f32 vdistSqr = vofs.length2();

				CParticleVector foffset((u + v) - offset);
				CParticleVector fofs = (un * (un * foffset)) - foffset;
				f32 fdistSqr = fofs.length2();
				CParticleVector gofs = (un * (un * foffset)) - foffset;
				...
				// Blend S into V.
				CParticleVector tmp = (S * (magdt / (t * t + epsilon))) + Vn;
				m.vel = tmp * (vm / tmp.length());
			}
		}
		break;
		case ePDTriangle:
		{
			...
			CParticleVector f = v - u;
			CParticleVector fn(f);
			fn.normalize_safe();
			...
			for (u32 i = 0; i < effect->p_count; i++)
			{
				...
				CParticleVector vofs = (vn * (vn * offset)) - offset;
				f32 vdistSqr = vofs.length2();
				CParticleVector foffset(offset - u);
				CParticleVector fofs = (fn * (fn * foffset)) - foffset;
				f32 fdistSqr = fofs.length2();
				CParticleVector S;
		...
		case ePDSphere:
		{
			...
			for (u32 i = 0; i < effect->p_count; i++)
			{
				...
				CParticleVector L = position.p1 - m.pos;
				f32 v = L * Vn;
				...
				CParticleVector C = Vn ^ L;
				C.normalize_safe();
				CParticleVector S = Vn ^ C;
...
void PABounce::Execute(SParticleEffect* effect, f32 dt)
{
	switch (position.type)
	{
		case ePDTriangle:
		{
			...
			for (u32 i = 0; i < effect->p_count; i++)
			{
				...
				// See if particle's current and next positions cross plane.
				// If not, couldn't bounce, so keep going.
				CParticleVector pnext(m.pos + m.vel * dt);
				...
				CParticleVector vn(position.p2 * nv); // Normal Vn = (V.N)N
		...
		case ePDPlane:
		{
			...
			for (u32 i = 0; i < effect->p_count; i++)
			{
				...
				CParticleVector vn(position.p2 * nmag); // Normal Vn = (V.N)N
				CParticleVector vt(m.vel - vn); // Tangent Vt = V - Vn
		...
		case ePDSphere:
		{
			...
			for (u32 i = 0; i < effect->p_count; i++)
			{
				...
				if (position.Within(pnext))
				{
					...
					// position to surface.
					CParticleVector n(m.pos - position.p1);
					n.normalize_safe();
...
void PACopyVertexB::Execute(SParticleEffect* effect, f32 dt)
{
	u32 i;
...
// Dampen velocities
void PADamping::Execute(SParticleEffect* effect, f32 dt)
{
	// This is important if dt is != 1.
	CParticleVector one(1, 1, 1);
	CParticleVector scale(one - ((one - damping) * dt));
...
void PAExplosion::Execute(SParticleEffect* effect, f32 dt)
{
	...
	for (u32 i = 0; i < effect->p_count; i++)
	{
		...
		CParticleVector dir(m.pos - center);
...
void PAFollow::Execute(SParticleEffect* effect, f32 dt)
{
	f32 magdt = magnitude * dt;
	...
	if (max_radiusSqr < P_MAXFLOAT)
	{
		for (u32 i = 0; i < effect->p_count - 1; i++)
		{
			...
			CParticleVector tohim(effect->particles[i + 1].pos - m.pos); // tohim = p1 - p0
...
void PAGravitate::Execute(SParticleEffect* effect, f32 dt)
{
	f32 magdt = magnitude * dt;
	f32 max_radiusSqr = max_radius * max_radius;
	...
	if (max_radiusSqr < P_MAXFLOAT)
	{
		for (u32 i = 0; i < effect->p_count; i++)
		{
			...
			{
				SParticle& mj = effect->particles[j];

				CParticleVector tohim(mj.pos - m.pos); // tohim = p1 - p0
				f32 tohimlenSqr = tohim.length2() + EPS_S;

				if (tohimlenSqr < max_radiusSqr)
				{
					// Compute force exerted between the two bodies
					CParticleVector acc(tohim * (magdt / (_sqrt(tohimlenSqr) * (tohimlenSqr + epsilon))));
...
void PAGravity::Execute(SParticleEffect* effect, f32 dt)
{
	CParticleVector ddir(direction * dt);
...
void PAJet::Execute(SParticleEffect* effect, f32 dt)
{
	f32 magdt = magnitude * dt;
	f32 max_radiusSqr = max_radius * max_radius;
...
void PAScatter::Execute(SParticleEffect* effect, f32 dt)
{
	f32 magdt = magnitude * dt;
	f32 max_radiusSqr = max_radius * max_radius;
...
void PAKillOld::Execute(SParticleEffect* effect, f32 dt)
{
...
void PAMatchVelocity::Execute(SParticleEffect* effect, f32 dt)
{
	f32 magdt = magnitude * dt;
	f32 max_radiusSqr = max_radius * max_radius;
	...
	if (max_radiusSqr < P_MAXFLOAT)
	{
		for (u32 i = 0; i < effect->p_count; i++)
		{
			...
			for (u32 j = i + 1; j < effect->p_count; j++)
			{
				...
				{
					// Compute force exerted between the two bodies
					CParticleVector acc(mj.vel * (magdt / (tohimlenSqr + epsilon)));
...
void PAMove::Execute(SParticleEffect* effect, f32 dt)
{
...
void PAOrbitLine::Execute(SParticleEffect* effect, f32 dt)
{
	f32 magdt = magnitude * dt;
	f32 max_radiusSqr = max_radius * max_radius;
	...
	if (max_radiusSqr < P_MAXFLOAT)
	{
		for (u32 i = 0; i < effect->p_count; i++)
		{
			...
			CParticleVector f(m.pos - p);

			CParticleVector w(axis * (f * axis));

			// Direction from particle to nearest point on line.
			CParticleVector into = w - f;
...
void PAOrbitPoint::Execute(SParticleEffect* effect, f32 dt)
{
	f32 magdt = magnitude * dt;
	f32 max_radiusSqr = max_radius * max_radius;
	...
	if (max_radiusSqr < P_MAXFLOAT)
	{
		for (u32 i = 0; i < effect->p_count; i++)
		{
			...
			// Figure direction to particle.
			CParticleVector dir(center - m.pos);
...
void PARandomAccel::Execute(SParticleEffect* effect, f32 dt)
{
	for (u32 i = 0; i < effect->p_count; i++)
	{
		SParticle& m = effect->particles[i];

		CParticleVector acceleration;
...
void PARandomDisplace::Execute(SParticleEffect* effect, f32 dt)
{
	for (u32 i = 0; i < effect->p_count; i++)
	{
		SParticle& m = effect->particles[i];

		CParticleVector displacement;
...
void PARandomVelocity::Execute(SParticleEffect* effect, f32 dt)
{
	for (u32 i = 0; i < effect->p_count; i++)
	{
		SParticle& m = effect->particles[i];

		CParticleVector velocity;
...
void PARestore::Execute(SParticleEffect* effect, f32 dt)
{
	if (time_left <= 0)
	{
		for (u32 i = 0; i < effect->p_count; i++)
...
void PASink::Execute(SParticleEffect* effect, f32 dt)
{
	// Must traverse list in reverse order so Remove will work
	for (int i = effect->p_count - 1; i >= 0; i--)
	{
...
void PASinkVelocity::Execute(SParticleEffect* effect, f32 dt)
{
	// Must traverse list in reverse order so Remove will work
	for (int i = effect->p_count - 1; i >= 0; i--)
	{
...
void PASource::Execute(SParticleEffect* effect, f32 dt)
{
...
void PASpeedLimit::Execute(SParticleEffect* effect, f32 dt)
{
...
void PATargetColor::Execute(SParticleEffect* effect, f32 dt)
{
...
void PATargetSize::Execute(SParticleEffect* effect, f32 dt)
{
	...
	{
		SParticle& m = effect->particles[i];
		CParticleVector dif(size - m.size);
		dif.x *= scaleFac_x;
		dif.y *= scaleFac_y;
		dif.z *= scaleFac_z;
		m.size += dif;
...
void PATargetRotate::Execute(SParticleEffect* effect, f32 dt)
{
	f32 scaleFac = scale * dt;

	CParticleVector r = CParticleVector(_abs(rot.x), _abs(rot.y), _abs(rot.z));

	for (u32 i = 0; i < effect->p_count; i++)
	{
		SParticle& m = effect->particles[i];
		//		m.rot += (r - m.rot) * scaleFac;
		CParticleVector sign(m.rot.x >= 0.f ? scaleFac : -scaleFac, m.rot.y >= 0.f ? scaleFac : -scaleFac, m.rot.z >= 0.f ? scaleFac : -scaleFac);
		CParticleVector dif((r.x - _abs(m.rot.x)) * sign.x, (r.y - _abs(m.rot.y)) * sign.x, (r.z - _abs(m.rot.z)) * sign.x);
		m.rot += dif;
	}
}
...
void PATargetVelocity::Execute(SParticleEffect* effect, f32 dt)
{
	f32 scaleFac = scale * dt;
...
void PAVortex::Execute(SParticleEffect* effect, f32 dt)
{
	f32 magdt = magnitude * dt;
	f32 max_radiusSqr = max_radius * max_radius;
	...
	if (max_radiusSqr < P_MAXFLOAT)
	{
		for (u32 i = 0; i < effect->p_count; i++)
		{
			...
			CParticleVector offset(m.pos - center);
			...
			CParticleVector offnorm(offset / r);
			...
			CParticleVector w(axis * axisProj); // parallel component
			CParticleVector u(offnorm - w); // perpendicular component

			// Perpendicular component completing frame:
			CParticleVector v(axis ^ u);
...
// Turbulence
#include "noise.h"
void PATurbulence::Execute(SParticleEffect* effect, f32 dt)
{
	CParticleVector pV;
	CParticleVector vX;
	CParticleVector vY;
