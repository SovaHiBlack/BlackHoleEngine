...
static CParticleVector vHalf(0.5, 0.5, 0.5);

ICF CParticleVector RandVec()
{
	return CParticleVector(drand48(), drand48(), drand48());
}
...
////////////////////////////////////////////////////////////////////////////////
// Stuff for the SParticleDomain.
SParticleDomain::SParticleDomain(EParticleDomain dtype, f32 a0, f32 a1,
								 ...
								 f32 a6, f32 a7, f32 a8)
{
	...
	switch (type)
	{
		case ePDPoint:
		{
			p1						= CParticleVector(a0, a1, a2);
		}
		break;
		case ePDLine:
		{
			p1						= CParticleVector(a0, a1, a2);
			CParticleVector tmp		(a3, a4, a5);
		...
		{
			p1						= CParticleVector(a0, a1, a2);
			CParticleVector tp2		= CParticleVector(a3, a4, a5);
			CParticleVector tp3		= CParticleVector(a6, a7, a8);
			...
			radius1Sqr				= u.length();
			CParticleVector tu		= u / radius1Sqr;
			radius2Sqr				= v.length();
			CParticleVector tv		= v / radius2Sqr;
		...
		{
			p1						= CParticleVector(a0, a1, a2);
			u						= CParticleVector(a3, a4, a5);
			v						= CParticleVector(a6, a7, a8);
		...
		case ePDCylinder:
		{
			...
			CParticleVector basis	(1.0f, 0.0f, 0.0f);
			if (_abs(basis * n) > 0.999)
			{
				basis				= CParticleVector(0.0f, 1.0f, 0.0f);
			}
		...
		case ePDDisc:
		{
			p1						= CParticleVector(a0, a1, a2);		// Center point
			p2						= CParticleVector(a3, a4, a5);		// Normal (not used in Within and Generate)
			p2.normalize_safe		();
...
BOOL SParticleDomain::Within(const CParticleVector& pos) const
{
	switch (type)
	{
		case ePDBox:
		...
		case ePDCone:
		{
			...
			CParticleVector xrad = x - p2 * dist; // Radial component of x
...
void SParticleDomain::Generate(CParticleVector& pos) const
{
	switch (type)
	{
		case ePDPoint:
		...
		break;
		case ePDPlane: // How do I sensibly make a point on an infinite plane?
		{
			pos = p1;
		}
		break;
		...
		default:
		{
			pos = CParticleVector(0, 0, 0);
		}
	}
}

void SParticleDomain::transform(const SParticleDomain& domain, const Fmatrix& m)
{
	switch (type)
	{
		case ePDBox:
...
void SParticleDomain::transform_dir(const SParticleDomain& domain, const Fmatrix& m)
{
	Fmatrix M = m;
	M.c.set(0, 0, 0);
	transform(domain, M);
