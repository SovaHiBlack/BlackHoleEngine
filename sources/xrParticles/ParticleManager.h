...
namespace PAPI
{
	class CParticleManager : public IParticleManager
	{
		...
		DEFINE_VECTOR(SParticleEffect*, ParticleEffectVec, ParticleEffectVec_it);
		DEFINE_VECTOR(CParticleActions*, ParticleActionsVec, ParticleActionsVec_it);
	...
	public:
		...
		virtual void				SetCallback				(int effect_id, OnBirthParticleCB b, OnDeadParticleCB d, pvoid owner, u32 param);
		virtual void				GetParticles			(int effect_id, SParticle*& particles, u32& cnt);
		virtual u32					GetParticlesCount		(int effect_id);

		// action
		virtual SParticleAction*	CreateAction			(EParticleAction type);
